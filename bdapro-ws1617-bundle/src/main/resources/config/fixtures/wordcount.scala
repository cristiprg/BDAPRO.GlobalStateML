...
/** `WordCount` experiment fixtures for the 'bdapro-ws1617' bundle. */
@Configuration
  ...
  def `datagen.words`: FlinkJob = new FlinkJob(
    ...
    command =
      """
        ...
        |${app.path.datagens}/bdapro-ws1617-datagens-1.0-SNAPSHOT.jar        \
    ...
    val `wordcount.flink.default` = new FlinkExperiment(
      ...
      command =
        """
          ...
          |${app.path.apps}/bdapro-ws1617-flink-jobs-1.0-SNAPSHOT.jar          \
          |${system.hadoop-2.path.input}/rubbish.txt                            \
    ...
    val `wordcount.spark.default` = new SparkExperiment(
      ...
      command =
        """
          ...
          |${app.path.apps}/bdapro-ws1617-spark-jobs-1.0-SNAPSHOT.jar          \
          |${system.hadoop-2.path.input}/rubbish.txt                            \
          |${system.hadoop-2.path.output}/wordcount
        """.stripMargin.trim,
      config  = ConfigFactory.parseString(""),
